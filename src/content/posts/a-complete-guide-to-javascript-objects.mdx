...
This article may skip on a few facts (because this is not a documentation). If you want to read the docs, go to [MDN](https://developer.mozilla.org/en-US/).
...
If you have _something_ in JavaScript (labelled by property or a variable), it is a value, meaning an _object_ or a _primitive_. A value has a type (and associated data according to that type). You can test for that type, but the associated data is what we're modifying implicitly through operations.
...
This last statement is very important because it is the source of many side effect bugs in JavaScript code.
...
If you want to use objects as maps, please just use the built-in `Map`. Objects perform bad as key-value stores for _random and dynamic_ data.
...
The one that I'm most mad about is the one that is the most standard in any programming language: _sum_ types. The ability to group other types in JavaScript is implemented with classes. Classes classically have polymorphic semantics, and they also in JS.
