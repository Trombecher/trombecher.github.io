...
description: "Dive in the deep web of text encodings."
tags: ["articles"]
---

Text is one of the most fundamental types of media and it has multiplied in traffic over the years. The internet played a big part in this. Millions of websites flooded the internet, containing text. But how do you even store text?

Sure. In a text file, which you can open with any text editor, like _VS Code_ or _Sublime_ (_Microsoft Word_ does not count). But, since files are just bytes (at the core), somehow these bytes are interpreted as text. And in this article, we'll explore these interpretations.

## What Makes Text Text?

A text file is usually distinguished from a different file type by an extension or some other method of tagging, like [MIME types](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types/Common_types). This needs to happen because even if the file might be validly interpretated as text, in actuality it might be some other kind of format.
...
With ASCII, you have no chance of encoding more (advanced) characters 😭. This is the origin of [_ASCII Art_](https://en.wikipedia.org/wiki/ASCII_art), a whole branch of computer art derived from these 95 printable ASCII characters. These include smileys :-), angry smileys >:( and cute smileys :3. And of course tons of other things...

To encode more characters, people strived to create more text encodings. But before we go any further, we'll have to define some things:
...
- A _code unit_ is a collection of bits that repesent one character.

## The Split Of Coding And Encoding

If you payed close attention, you might have noticed that we actually made two steps in encoding text.
...
Suppose, you are the decoder, trying to decode a UTF-16 text stream into the corresponding Unicode code points. How do you know if the a character was encoded using one or two UTF-16 code units. This is impossible with our current knowledge, since all 2¹⁶ values of the single code unit are occupied with characters. There needs to be a range in the BMP that is invalid, a reserved range which no characters are mapped to, the [UTF-16 surrogate range](https://en.wikipedia.org/wiki/Universal_Character_Set_characters#Surrogates).
...
This UTF-16 design decision enforced Unicode to reserve the surrogate range and dictated the Unicode character set length to be 2²⁰ + 0x10000 - 0x800 = 1112064 characters.
...
As of efficiency, UTF-16 is double the size most of the time, because the most frequent symbols encountered are part of the english language, which does not require two bytes per symbol, but rather one (see the [ASCII section](#ascii)). But ASCII only supports 128 symbols; if only there was similar encoding capable of encoding all of Unicode...

### UTF-8

UTF-8 is _the_ standard of text encoding. And it is very straightforward.

import TextEncoderDecoder from "@/comp/TextEncoderDecoder.astro";
