...
const bufferToStrings = (buf: Uint8Array) => Array.from(buf, byte => byte.toString(16).padStart(2, "0"));

const textEncoder = new TextEncoder();

export const SUPPORTED_TEXT_ENCODINGS: [string, (str: string) => string[]][] = [
    ["Unicode Code Points", str => str.split("").map(c => `U+${c.charCodeAt(0)!.toString(16).padStart(4, "0")}`)],
    ["ASCII", str => {
